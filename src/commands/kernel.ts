  ...
  if (unknown.length > 0) {
    throw new CliError(`未知的选项: ${unknown.join(', ')}`, {
      ...
      hint: [
        '',
        '可用选项:',
        '  --mirror [镜像]   强制走镜像（默认 v6.gh-proxy.org），并记住偏好',
        '  --no-mirror       强制直连（绕过 gh/代理自动通道），并清除镜像偏好',
        '',
...
export async function cmdKernel(args: string[]): Promise<void> {
  ...
  // 显式 --mirror 记住偏好（下次裸 mihomo kernel 在 gh/代理都不可用时默认走镜像）；--no-mirror 清除
  ...
  if (channel.kind === 'gh') {
    ...
    const note = mirrorInfo.remember
      ? '已记住偏好：gh/本机代理不可用时默认走镜像（--no-mirror 清除偏好）'
      : !mirrorInfo.isOverride
        ? '已记住的偏好：gh/本机代理不可用时生效（--no-mirror 清除偏好）'
        : '仅本次下载走镜像';
  ...
  try {
    ...
    // 镜像仍绝不碰 API。--no-mirror/--direct 强制直连（含 API），绕过代理
    ...
    if (!effectiveMirror) {
      if (proxyRunning && !forceDirect) {
        hint.push('', '提示: 经本机代理查询 GitHub 失败，可检查代理状态（mihomo status），或 mihomo kernel --no-mirror 重试直连');
      } else {
        // 平时不打扰；仅直连失败时提示镜像用法
        hint.push(
          '',
          '提示: 直连失败或下载过慢时可使用镜像:',
          '  mihomo kernel --mirror [镜像]   # 强制走镜像（默认 v6.gh-proxy.org），一次使用后记住偏好',
          `  可用镜像: ${AVAILABLE_MIRRORS.join(', ')}`,
        );
      }
    }
    ...
    try {
      ...
      throw new CliError((e as Error).message, {
        ...
        hint: [
          ...
          '手动指定: mihomo kernel --mirror [镜像]（强制镜像）/ mihomo kernel --no-mirror（强制直连）',
        ],
      });
    }
    console.log(`\n已更新到 ${result.version}`);
