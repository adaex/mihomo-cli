...
describe('RESET_TARGETS 执行顺序', () => {
  ...
  // 这是 CODE_REVIEW #14 记录过的坑，ssh 隧道落地时又踩了一次，故用测试锁住。
  for (const id of ['subs', 'ssh']) {
  ...
  it('ssh 有 onBefore：必须先停进程再删运行态文件', () => {
    // 反序会让 ssh 进程失联——文件一删就再也找不到那些 PID，
    // 它们继续占着端口跑下去且 CLI 无任何路径能停掉
    const ssh = RESET_TARGETS.find(t => t.id === 'ssh');
    assert.ok(ssh?.onBefore, 'ssh 应有 onBefore 以先停进程');
  });
