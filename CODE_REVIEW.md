...
当前审查：2026-09-13，v4.11.0 后的全仓复审（未发布）

本轮是一次通读式复审，不针对某个功能。修 8 项：`listeners`/`tunnels` 入站锁定与补全安装覆盖（两项安全）、status 主文件显示名、`--mirror` 提示指向不存在的命令、zsh 补全 eval 模式不注册、bash 半截标记块死锁、zsh/fish 描述转义（两项预防性），以及 doctor 的 npm 查询串行（性能）；另收口两处硬编码词表、把 CHANGELOG 的篇幅纪律写进发布流程。单测 678（+21）
...
| 既有防线回归 | typecheck / 678 测试 / Biome（`src/` 81 文件）/ build 全绿；临时数据目录、测试 plist、进程均已清理核实 |
