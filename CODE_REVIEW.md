...
当前审查：2026-09-13，v4.11.0（status 覆写行按 match 区分是否适用当前订阅）

本轮改的是展示层一处误导：status 的覆写行此前列「目录里未被 `enabled: false` 停用的文件」，不按 match 过滤，只对别的订阅生效的文件与真正生效的混在同一行、形态完全相同，用户会拿它解释自己观察到的行为。修复后主行只列本次参与合并的文件，未命中的逐个展开原因。复查自己上一条提交时又发现 `status --json` 新增的 `applied` 漏了全局开关这道过滤，导致 `ow off` 后同一份 JSON 里 `enabled:false` 却列着生效文件、与人读形态打架，一并修掉。单测 657（+8）
...
## 本轮验证（v4.11.0 status 覆写行按 match 分列）
