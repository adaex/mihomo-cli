...
当前审查：2026-09-13，v4.10.0（覆写 match 的 `name` 通配与文件内 `enabled` 开关）

本轮在实现两项覆写增强的同时，连带堵上元数据键的操作符/大小写绕过、`*` 开头值的静默跳过，以及 glob 实现自身的灾难性回溯（后两者为复审时实测发现，其中回溯一条推翻了本文件初版写下的「输入面受控、可接受」结论）。单测 649（+46），验证结论见下方「覆写作用域与单文件开关」一行。
...
## 本轮验证（v4.10.0 覆写 match name 通配与文件内 enabled）
