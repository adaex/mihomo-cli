...
本轮是一次通读式复审，不针对某个功能。修 4 项：`listeners`/`tunnels` 入站锁定（安全）、status 主文件显示名、`--mirror` 重复时指向不存在命令的提示，以及 doctor 的 npm 查询串行（性能）；另把 CHANGELOG 的篇幅纪律写进发布流程。单测 665（+8）
...
| 既有防线回归 | typecheck / 665 测试 / Biome（`src/` 81 文件）/ build 全绿；临时数据目录、测试 plist、进程均已清理核实 |
