...
> 状态：**已实现**（v3.8.0），**v3.9.0 重构**。实现落点见 `src/ssh.ts`（进程侧）
> + `src/ssh-config.ts`（配置侧）+ `src/commands/ssh.ts`，
> 五个设计问题的最终答复见文末「实现记录」。
>
> **本文档记录的是 v3.8.0 当时的设计与实测结论，正文保留原貌。** v3.9.0 改了两处：
> 命令名 `tunnel` 统一为 `ssh`（无别名），配置文件 `overwrite.tunnel-<名字>.yaml`
> 改为独立于覆写开关的 `ssh.<名字>.yaml` 且节点改由 CLI 内建注入——
> 这一改动**解决了「实现记录 2」记载的局限**，详见该节的补记。
...
> **v3.9.0 补记：本条局限已解决。** 做法不是扩展覆写语法，而是取消「节点写在用户文件里」
> 这个前提——socks5 节点改由 CLI 依据 settings 的 host/port 内建注入，且**注入恒排在
> 所有用户文件之后**。`~` 的「后合并者胜」于是反过来成了保障：用户即便声明同名节点，
> 也覆盖不掉 `server`/`port`（连绕过 `127.0.0.1` 绑定都不行）。有单测锁定该顺序。
>
> 同时修掉了一个当时未发现的 bug：节点在用户文件、端口在 settings，构成两份真相——
> `tunnel rm` 后换端口 `add`，模板已存在故不重建，配置仍指向旧端口且无任何提示，**静默失效**。
...
> **v3.9.0 补记：文件归属重新划分。** `ssh.*.yaml` 不再匹配 `isOverwriteFilename`，
> 于是 `reset ow` 不再碰它（这正是解耦要的）。但随之出现新问题：两个目标都不管的话，
> `reset --full` 声称「删全部」却会留下配置文件。故 `reset ssh` 目标改为连
> `ssh.*.yaml` 一并删。**单条 `ssh rm` 仍不删**——两者语义不同：前者是显式的整体重置
> 且带确认，后者只是移除一条隧道，用户手写的分流规则不该跟着消失。
...
> **v3.9.0 补记：本条结论已推翻。** 主名改为 `ssh`，`tunnel`/`tunnels` 别名全部删除——
> 同一功能两个名字，而磁盘上（目录、日志、类型、settings 字段）全是 tunnel，
> 命令面却多出个 ssh，属于命名分裂。改名时那条「重复 token 启动即炸」的提醒仍然有效：
> 主名换成 `ssh` 的同时必须把 `aliases` 里的 `'ssh'` 删掉。`tun` 依旧不可用作别名。
