...
- 入站端口与整个控制面是系统锁定项：订阅/覆写显式设置时一律剥除，但**只对生效的覆写文件告警**（订阅侧静默——机场订阅普遍自带端口段，系统约束接管是核心设计、用户无行动手段，告警只会刷屏；亲手写覆写的用户以为生效才需要提示，告警含文件名）。锁定键以 `config.ts` 的 `LOCKED_CONFIG_KEYS` 为唯一清单：mixed-port/port/socks-port/redir-port/tproxy-port、external-controller 全家桶（-tls/-unix/-pipe/-cors/-routing-mark/-doh）、tuic-server（自带证书字段的完整入站代理，可把本机变开放代理）、external-ui*、secret；顶层 `tls` 段（-tls 控制器的证书来源）同锁（剥除不看来源、告警同样只对覆写，含 +key/key! 操作符形式）。新增入站/控制器键只改这张表——redir/tproxy、-tls/-unix/-doh、tuic-server 都曾是漏网之鱼（信任边界不是配置洁癖，每个键都要回上游 General 段核对）。secret 只取 settings.controller_secret 且必须是字符串（非字符串在 buildConfig fail-closed，同 getPorts）；`listeners` 与 `tunnels`（通用入站声明）刻意不锁、须一起评估，`iptables` 是 Linux 专用非监听（决策见 CODE_REVIEW）
