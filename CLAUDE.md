...
- 入站端口与整个控制面是系统锁定项：订阅/覆写显式设置时一律剥除，但**只对生效的覆写文件告警**（订阅侧静默——机场订阅普遍自带端口段，系统约束接管是核心设计、用户无行动手段，告警只会刷屏；亲手写覆写的用户以为生效才需要提示，告警含文件名）。锁定键以 `config.ts` 的 `LOCKED_CONFIG_KEYS` 为唯一清单：mixed-port/port/socks-port/redir-port/tproxy-port、external-controller 全家桶（-tls/-unix/-pipe/-cors/-routing-mark/-doh）、tuic-server 与 ss-config/vmess-config（三个完整入站代理服务端，自带监听地址与认证，可把本机变开放代理）、external-ui*、secret；顶层 `tls` 段（-tls 控制器的证书来源）同锁（剥除不看来源、告警同样只对覆写，含 +key/key! 操作符形式）。新增入站/控制器键只改这张表——redir/tproxy、-tls/-unix/-doh、tuic-server、ss-config/vmess-config 都曾是漏网之鱼（信任边界不是配置洁癖）。核对方法不是按键名眼熟程度挑，而是回上游 `config.Inbound` 结构体字段全集 + `executor.updateListeners()` 里逐个 ReCreate* 的入参对表：进得去那份名单的都能开监听。别拿 allow-lan/bind-address 当兜底——上游 genAddr 只作用于 HTTP/Socks/Redir/TProxy/Mixed，ss/vmess/tuic 自带 Listen 不受其约束。secret 只取 settings.controller_secret 且必须是字符串（非字符串在 buildConfig fail-closed，同 getPorts）；`listeners` 与 `tunnels`（通用入站声明）刻意不锁、须一起评估，`iptables` 是 Linux 专用非监听（决策见 CODE_REVIEW）
...
- match 的 `name` 与 `subscription` 是同义键，加载时归一到内部 `subscription` 字段，判据（matchesScope）只此一处；两者同时出现报错（同义键无法判断以谁为准），原键名存 `subscriptionKey` 仅供展示——`ow list` 与内核拒绝提示回显用户写的键名，显示一个文件里搜不到的键名会让人找不到源头。订阅名支持 `*`/`?` glob：全串匹配（`edu*` 不命中 `xedu1`）、无通配字符时退化为精确比对（老写法行为不变）、正则元字符字面。转义与通配替换必须在同一次 replace 内完成，先转义再放开会把字面 `\*` 一并当通配；`*` 先折叠故无嵌套回溯，订阅名另受 SAFE_NAME_RE 限长 64
