...
- 入站端口与整个控制面是系统锁定项：订阅/覆写显式设置时一律剥除，但**只对生效的覆写文件告警**（订阅侧静默——机场订阅普遍自带端口段，系统约束接管是核心设计、用户无行动手段，告警只会刷屏；亲手写覆写的用户以为生效才需要提示，告警含文件名）。锁定键以 `config.ts` 的 `LOCKED_CONFIG_KEYS` 为唯一清单：mixed-port/port/socks-port/redir-port/tproxy-port、external-controller 全家桶（-tls/-unix/-pipe/-cors/-routing-mark/-doh）、tuic-server 与 ss-config/vmess-config（三个完整入站代理服务端，自带监听地址与认证，可把本机变开放代理）、listeners/tunnels（通用入站声明，同样自带监听地址）、allow-lan/bind-address/authentication/skip-auth-prefixes/lan-allowed-ips/lan-disallowed-ips（局域网暴露与入站鉴权，见下）、external-ui*、secret；顶层 `tls` 段（-tls 控制器的证书来源）同锁（剥除不看来源、告警同样只对覆写，含 +key/key! 操作符形式）。新增入站/控制器键只改这张表——redir/tproxy、-tls/-unix/-doh、tuic-server、ss-config/vmess-config、listeners/tunnels、allow-lan 与鉴权家族都曾是漏网之鱼（信任边界不是配置洁癖）。核对方法不是按键名眼熟程度挑，而是回上游 `config.Inbound` 结构体字段全集 + `executor.updateListeners()` 里逐个 ReCreate* 的入参对表：进得去那份名单的都能开监听。**allow-lan 与 bind-address 自 v4.13.0 起自己就是锁定项**（此前只写着「别拿它们当兜底」，却没锁它们自身——上游 genAddr 在 allow-lan 为真、bind-address 为默认 `*` 时返回 `":port"`，订阅一行就把 Mixed 端口挪出回环；而 skip-auth-prefixes 命中后 `http/server.go` 把鉴权 store 换成 authStore.Nil，让 authentication 整个失效）；「genAddr 只作用于 HTTP/Socks/Redir/TProxy/Mixed，ss/vmess/tuic 自带 Listen 不受其约束」这条结论仍成立，仍不能拿 allow-lan 当那三个键的兜底。allow-lan 的恒定值 false 由 `buildConfig` 的 systemConfig 写入，**不放 BASE_CONFIG**：后者语义是「用户没写时的默认」、会被剥除循环架空成死配置，两表无交集有测试锁死。secret 只取 settings.controller_secret 且必须是字符串（非字符串在 buildConfig fail-closed，同 getPorts）；`iptables`（Linux 专用非监听）与 `inbound-tfo`/`inbound-mptcp`（传输层 socket 选项，不开监听、不改绑定地址、不绕鉴权）刻意不锁，`tun` 由启动模式整段接管、也不经这张表（决策见 CODE_REVIEW）。「待定」不是中间状态而是放行：`listeners`/`tunnels` 以「未定的产品决策」挂了三个版本，期间订阅写了就照常进运行配置，v4.12.0 按同一判据收口
- 锁定清单的完整性有机制兜底：`config-inbound-snapshot.spec.ts` 存了一份带上游版本号的 `config.Inbound` 字段快照，凡不在锁定表里的字段必须在 `NOT_IN_LOCKED_TABLE` 写明放行理由，否则测试红。**但快照是冻结副本、发现不了上游新增字段**——内核大版本升级时人工刷新该快照，方法同上（照结构体字段全集 + updateListeners 对表）
...
- 覆写主文件先加载，扩展文件按名称排序；主文件名只认 overwrite.yaml，`overwrite.yml` 这类整体近失（小写比对后与合法形态一致）在加载/列表入口 console.warn 一行，只认整体近失、宁可漏报不误报；match 的订阅名键与 url-domain 为 AND 条件，订阅名匹配不区分大小写，无 match 全局应用，非法或空 match 报错
